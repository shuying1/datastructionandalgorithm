...
public class FibonacciSearch {
    ...
    public static int fibSearch(int[] arr, int findValue) {
        ...
        //因为 f[k] 值 可能大于 a 的 长度，因此我们需要使用Arrays 类，构造一个新的数组，并指向 temp[]
        // 不足的部分会使用 0 填充
        int tmp[] = Arrays.copyOf(arr, f[k]);
        //实际上需求使用 a 数组最后的数填充 temp
        // 举例:
        //temp = {1,8, 10, 89, 1000, 1234, 0, 0} => {1,8, 10, 89, 1000, 1234, 1234, 1234,}
        for (int i = high; i < tmp.length; i++) {
            tmp[i] = arr[high];
        }
        while (low <= high) {// 只要这个条件满足，就可以找
            mid = low + f[k - 1] - 1;
            if (findValue < tmp[mid]) {// 只要这个条件满足，就可以找
                high = mid - 1;
                k--;
                //为什么是 k--
                // 说明
                ...
                //即 在 f[k-1] 的前面继续查找 k--
                // 即下次循环 mid = f[k-1-1]-1
            } else if (findValue > tmp[mid]) {// 我们应该继续向数组的后面查找(右边)
                low = mid + 1;
                k -= 2;
                //为什么是 k -=2
                // 说明
