...
class ArrayStack2 {
    ...
    // 假定目前的表达式只有 +, - , * , /
    //字符底层是ASCII 为数值
    public int optPriority(char opt) {
        //直接return 不需要break的
        switch (opt) {
            case '*':
            case '/':
...
public class InfixExpressionCalculator {
    ...
    public static void main(String[] args) {
        String expression = "7*2*10"; //如何处理多位数的问题？
        //创建两个栈，数栈，一个符号栈
        ...
        for (int i = 0; i < expression.length(); i++) {
            ...
            //判断 ch 是什么，然后做相应的处理
            if (operStack.isOpt(ch)) {//如果是运算符
                if (!operStack.isEmpty()) {
                    //如果符号栈有操作符，就进行比较,如果当前的操作符的优先级小于或者等于栈中的操作符,就需要从数栈中 pop 出两个数,
                    //再从符号栈中 pop 出一个符号，进行运算，将得到结果，入数栈，然后将当前的操作符入符号栈
                    //这里要强转一下
                    if (operStack.optPriority(ch) <= operStack.optPriority((char) operStack.peek())) {
                        num1 = numStack.pop();
                        num2 = numStack.pop();
                        opt = (char) operStack.pop();
...
/因为expression.charAt(i)取出的为字符
                ...
                if (index == expression.length() - 1) {
                    ...
                    //注意是看后一位，不是index++
                    if (operStack.isOpt(expression.substring(index + 1, index + 2).charAt(0))) {
                        //如果后一位是运算符，则入栈
                        numStack.push(Integer.parseInt(keepNum));
                        //重要的!!!!!!, keepNum清空
                        keepNum = "";
